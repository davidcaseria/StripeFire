    ...
    return function (ref, callback, accessToken, alterRequest) {
        ...
        return {
            refunds: function (ref, callback, accessToken, alterRequest) {
                var refundsRef = new FireChild(ref);

                refundsRef.on(1, 'child_added', function (childSnapshot) {
                    ...
                    if (refundData.id) {
                        ...
                        var createRefund = function (refundRequest) {
                            var chargeName = childSnapshot.ref().parent().name();
                            
                            // Save response back at location
                            var saveRefund = function (err, refund) {
                                childSnapshot.ref().set(err ? err.raw : refund);
                                if (!err) {
                                    stripe.charges.retrieve(refund.charge, function (err, charge) {
                                        chargesRef.child(chargeName).set(err ? err.raw : charge);
                                    });
                                }
                            ...
                            chargesRef.child(chargeName).once('value', function (dataSnapshot) {
