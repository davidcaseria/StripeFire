    ...
    return function(childSnapshot) {
        ...
        if (data.id) {
            stripe[object.toLowerCase() + 's'].retrieve(data.id, function (err, obj) {
                ...
                childSnapshot.ref().set(err ? {err: err.raw} : obj);
            });
        } else {
            var create = function (request) {
                ...
                var handleResponse = function(err, obj) {
                    obj._hash = sha1(JSON.stringify(obj));
                    childSnapshot.ref().set(err ? {err: err.raw} : obj, function() {
                        if(callback) {
                            callback(err, obj);
                        }
                    });
    ...
    return function (childSnapshot) {
        ...
        var create = function (request) {
            ...
            var handleReponse = function(err, obj) {
                if(err) {
                    childSnapshot.ref().set({err: err.raw});
                    if(callback) {
                        callback(err);
                    }
                }
                    ...
                    stripe[object.toLowerCase() + 's'].retrieve(obj[object.toLowerCase()], function (err, obj) {
                        parentRef.child(parentName).set(err ? {err: err.raw} : obj, function() {
                            if (callback) {
                                callback(null, obj);
                            }
                        });
...
StripeObject.prototype.update = function(keys) {
    var stripe = this.stripe;
    var object = this.object;
